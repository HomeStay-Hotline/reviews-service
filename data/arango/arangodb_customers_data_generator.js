...
const filename = argv.output || 'arango_customers.csv';
const stream = fs.createWriteStream(filename);

const randomInt = () => Math.floor(Math.random() * 100);

const generateCustomerRecord = (customer_id) => {
    const customer_name = faker.name.findName();
    const photo_url = `${s3URL}${randomInt().toString().padStart(3, '0')}.jpg`;

    return `${customer_id},${customer_name},${photo_url}\n`
};
...
const startWriting = (writeStream, encoding, done) => {
    ...
    function writing(){
      let canWrite = true
      do {
        let record = generateCustomerRecord(i);
        i--;
        if(i === 0){
...
stream.write(`customer_id,customer_name,photo_url\n`, 'utf-8')
